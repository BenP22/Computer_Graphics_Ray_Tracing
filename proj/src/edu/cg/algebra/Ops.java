...
public class Ops {
	...
	public static Vec refract(Vec u, Vec normal, double n1, double n2) {
		...
		double dot = dot(neg(u), normal) * dot(neg(u), normal);

		if(n1 > n2) {
			double angle = (n2 / n1) * (n2 / n1);
		...
		Vec b = normalize(add(u, mult(dot(neg(u), normal), normal)));

		double sin2 = Math.sqrt((n1 * n1 * (1-dot)) / (n2*n2));
		double cos2 = Math.sqrt(1 - sin2);

		return add(mult(-cos2, normal), mult(sin2, b));
	}
}
